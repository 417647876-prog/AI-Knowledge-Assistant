...
async def purge_document(
    *, session_factory: SessionFactory, upload_directory: Path, lease: JobLease
) -> int:
    if lease.job_type != "purge_document" or lease.resource_type != "document":
        raise ValueError("purge_document 仅处理文档清理任务")
    async with session_factory() as session:
        try:
            return await _purge_document_in_session(session, upload_directory, lease)
...
async def _purge_document_in_session(
    session: AsyncSession, upload_directory: Path, lease: JobLease
) -> int:
    ...
    now = await _database_now(session)
    _validate_purge_time(document.deleted_at, document.purge_after, now)
...
async def purge_knowledge_base(
    *, session_factory: SessionFactory, upload_directory: Path, lease: JobLease
    ...
    async with session_factory() as session:
        try:
            return await _purge_knowledge_base_in_session(session, upload_directory, lease)
        except AppError as error:
            await _record_purge_denial(session, lease, error)
            raise
...
async def _purge_knowledge_base_in_session(
    ...
    now = await _database_now(session)
    _validate_purge_time(knowledge_base.deleted_at, knowledge_base.purge_after, now)
    documents = (
        await session.scalars(
            select(Document)
...
def _validate_purge_time(deleted_at, purge_after, now) -> None:
    if deleted_at is None or purge_after is None:
        raise AppError(
            code="RESOURCE_NOT_IN_TRASH",
            message="资源不在回收站中。",
