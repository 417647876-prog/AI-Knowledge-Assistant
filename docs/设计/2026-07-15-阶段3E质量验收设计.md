...
- 四份原始 JSON 继续由 Git 忽略；脱敏后的中文综合报告提交到 `docs/验收与演示/阶段3质量验收报告.md`。
...
正式报告路径固定为 `docs/验收与演示/阶段3质量验收报告.md`，章节顺序固定：
...
- 模型：`Sol｜xhigh`。
- 交付：真实报告、正式 Markdown、`docs/验收与演示/阶段3验证与演示.md`、README、学习笔记和执行看板。
