...
5. 每次切换都写入 `docs/实施计划/阶段3执行进度.md`，记录原模型、目标模型、失败验证和切换原因。
...
先读取 `docs/实施计划/阶段3执行进度.md`，必须同时看到“3C 已完成”和“3D 未开始”。如果 3C 不是已完成状态，停止本 Task，把 3D 保持为未开始，不修改业务代码。
