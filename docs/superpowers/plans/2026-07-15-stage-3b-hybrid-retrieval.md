...
总体 Recall@5 不低于 3A；关键词分类 Recall@5 达到
`min(100%, 纯向量关键词 Recall@5 + 10 个百分点)`；引用命中率和拒答准确率均不得低于
纯向量。该上限感知规则用于处理纯向量关键词基线已达 100% 的饱和情况，不修改评估数据。
未达标时保持 3B `进行中`，记录失败数据，不进入 3C。
...
通过后将 3B 标为 `已完成`，记录两份报告摘要、验证命令和提交标识。阶段 3C 保持
`未开始`，由后续独立任务启动。
