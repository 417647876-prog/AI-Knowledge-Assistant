...
| 工作内容 | 建议模型 | 推理强度 |
|---|---|---:|
| 数据库迁移、任务时间排序、文档列表 API | Terra | high |
| 删除事务、文件系统失败恢复、权限与问答回归 | Sol | high |
| Vue API、Pinia 状态管理 | Terra | high |
| 文档表格交互、组件测试 | Terra | medium |
| 全量验收 | Terra high；连续两轮无法定位时切换 Sol xhigh | high/xhigh |
| 最终安全复核 | Sol | high |

默认先用 Terra；涉及数据库与本地文件一致性、越权边界或跨层疑难问题时切换 Sol。阶段 2C 不需要 max 或 ultra。
