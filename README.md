...
`RAG_RETRIEVAL_MODE=hybrid` 才启用混合检索。
...
| hybrid | 86.67% | 83.33% | 80.00% | 86.67% |

总体 Recall@5 没有退化，但关键词分类的纯向量基线已经是 100%，混合检索仍为 100%，无法满足
“至少提升 10 个百分点”的原质量门；引用命中率也下降了 3.33 个百分点。因此 3B 暂不标记完成，
需要先新开任务确认质量门或调整评估集，并分析引用和拒答分类回归，之后才能进入 3C。
